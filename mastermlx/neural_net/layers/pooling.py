...
class GlobalAveragePooling1D(BaseLayer):
    ...
    def backward(self, grad):
        ...
        N, T, C = shape
        return np.broadcast_to(grad[:, None, :] / T, shape)
...
class GlobalAveragePooling2D(BaseLayer):
    ...
    def backward(self, grad):
        ...
        if shape is None:
            raise RuntimeError("forward must be called before backward")
        N, H, W, C = shape
        return np.broadcast_to(grad[:, None, None, :] / (H * W), shape)
...
class AvgPool2D(BaseLayer):
    ...
    def forward(self, X):
        ...
        for i in range(OH):
            for j in range(OW):
                out[:, i, j, :] = np.mean(
                    X[:, i * self.s : i * self.s + self.k, j * self.s : j * self.s + self.k, :],
                    axis=(1, 2),
                )
        return out
    ...
    def backward(self, grad):
        ...
        if shape is None:
            raise RuntimeError("forward must be called before backward")
        N, H, W, C = shape
        OH, OW = grad.shape[1:3]
        dX = np.zeros(shape, dtype=float)
        scale = 1.0 / (self.k * self.k)
        for i in range(OH):
            for j in range(OW):
                dX[:, i * self.s : i * self.s + self.k, j * self.s : j * self.s + self.k, :] += (
